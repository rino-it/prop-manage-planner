...
import { 
  ...
  LogOut, FileText, UserPlus, FolderOpen, Users, Truck, Ticket
} from 'lucide-react';
...
export default function Sidebar({ activeTab, setActiveTab, onCloseMobile }: SidebarProps) {
  ...
  return (
    <div className="flex h-full w-full flex-col bg-white border-r border-slate-200 pb-safe"> {/* pb-safe per mobile */}
      ...
      <ScrollArea className="flex-1 px-3 py-4">
        <nav className="space-y-1 pb-6"> {/* Extra padding per scroll */}
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-3 text-sm font-medium rounded-lg transition-all duration-200 group touch-manipulation", // touch-manipulation per mobile
                  isActive 
      ...
      <div className="p-4 border-t bg-slate-50/50 pb-6"> {/* Padding extra per barre gesture */}
        <Button 
          variant="outline" 
          className="w-full justify-start text-red-600 hover:text-red-700 hover:bg-red-50 border-red-100 h-10"
          onClick={() => signOut()}
        >
