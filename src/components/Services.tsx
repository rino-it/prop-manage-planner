...
export default function Services() {
  ...
  const [editingService, setEditingService] = useState<any>(null); // Stato per modifica
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold text-gray-900">Servizi & Esperienze</h1><p className="text-gray-500">Configura l'upselling.</p></div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {isLoading ? <p>Caricamento...</p> : services?.map((service) => (
          <Card key={service.id} className="overflow-hidden hover:shadow-lg transition-shadow flex flex-col group">
            <div className="h-48 bg-gray-100 relative">
              ...
              <div className="absolute top-2 left-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button size="icon" variant="secondary" className="h-8 w-8 bg-white/90 hover:bg-white" onClick={() => openEdit(service)}><Pencil className="w-4 h-4 text-blue-600" /></Button>
                  <Button size="icon" variant="secondary" className="h-8 w-8 bg-white/90 hover:bg-red-50" onClick={() => deleteService.mutate(service.id)}><Trash2 className="w-4 h-4 text-red-600" /></Button>
              </div>
            </div>
            <CardHeader>
