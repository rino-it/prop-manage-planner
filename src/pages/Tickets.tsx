...
import { 
  ...
  Upload, FileSpreadsheet, Trash2, Clock, Search, ChevronLeft, ChevronRight // ICONE AGGIUNTE
} from 'lucide-react';
import { format } from 'date-fns';
import { it } from 'date-fns/locale'; // IMPORTANTE PER DATE IN ITALIANO
...
export default function Tickets() {
  ...
  // Stati Filtri (MANTENGO I TUOI E AGGIUNGO I NUOVI)
  const [filterType, setFilterType] = useState('all'); // Il tuo filtro originale (Tutti/Imm/Veicoli)
  const [searchTerm, setSearchTerm] = useState(''); // Nuovo: Ricerca testo
  const [filterPriority, setFilterPriority] = useState('all'); // Nuovo: Filtro Priorità
  ...
  const deleteOrphans = async () => {
      ...
      try {
          const { error } = await supabase.from('tickets').delete().is('property_real_id', null).is('property_mobile_id', null);
          if (error) throw error;
          toast({ title: "Pulizia Completata", description: "Ticket orfani rimossi." });
          queryClient.invalidateQueries({ queryKey: ['tickets'] });
      } catch(e: any) { toast({ title: "Errore", description: e.message, variant: "destructive" }); } finally { setIsProcessing(false); }
  };
    ...
    reader.onload = async (e) => {
        try {
            ...
            for (const row of rows) {
                ...
                const finalDate = sanitizeDate(deadlineRaw);
                ...
                await supabase.from('tickets').insert({
                    titolo: title, descrizione: desc, priorita: 'media', stato: 'aperto', creato_da: 'manager',
                    user_id: user?.id, property_real_id: propId, scadenza: finalDate
                });
                importedCount++;
            }
            toast({ title: "Importazione Riuscita", description: `${importedCount} ticket creati.` });
            setIsImportOpen(false); setCsvFile(null); queryClient.invalidateQueries({ queryKey: ['tickets'] });
        } catch (err: any) { toast({ title: "Errore Importazione", description: err.message, variant: "destructive" }); } finally { setIsProcessing(false); }
    };
  ...
  const handleShortcutClick = (shortcut: any) => {
    ...
    if (found) {
      setTargetType('real'); setFormData({ ...formData, target_id: found.id });
      toast({ title: "Proprietà Selezionata", description: `${found.nome}` });
    } else { toast({ title: "Non trovata", description: `Nessuna proprietà per "${shortcut.name}"`, variant: "destructive" }); }
  };
  ...
  const createTicket = useMutation({
    mutationFn: async (newTicket: typeof formData) => {
      ...
      const payload: any = {
        titolo: newTicket.titolo, descrizione: newTicket.descrizione, priorita: newTicket.priorita,
        user_id: user?.id, creato_da: 'manager', stato: 'aperto',
        booking_id: newTicket.booking_id === 'none' ? null : newTicket.booking_id,
        assigned_to: newTicket.assigned_to, attachments: attachments, scadenza: newTicket.scadenza || null
      };
  ...
  const handleContactPartner = async (ticket: any, phone: string | null) => {
      ...
      setGeneratingPdfId(ticket.id); toast({ title: "Generazione PDF...", description: "Sto preparando la scheda intervento." });
      try {
  ...
  // --- FILTRI AVANZATI + PAGINAZIONE (AGGIUNTO) ---
  const filteredTickets = useMemo(() => {
    ...
    return tickets.filter((t: any) => {
        ...
        // 2. Filtro Tipo (Il tuo originale: Tutti / Immobili / Veicoli)
        ...
        // 3. Ricerca Testuale (NUOVO)
        if (searchTerm) {
            const lower = searchTerm.toLowerCase();
            const matchesTitle = t.titolo?.toLowerCase().includes(lower);
            const matchesDesc = t.descrizione?.toLowerCase().includes(lower);
            const matchesProp = t.properties_real?.nome?.toLowerCase().includes(lower) || 
                                t.properties_mobile?.veicolo?.toLowerCase().includes(lower);
            if (!matchesTitle && !matchesDesc && !matchesProp) return false;
        }

        // 4. Filtro Priorità (NUOVO)
        if (filterPriority !== 'all' && t.priorita !== filterPriority) return false;

        return true;
    });
  }, [tickets, activeTab, filterType, searchTerm, filterPriority]);

  // KPI Rapidi
  const openCount = tickets?.filter((t:any) => t.stato !== 'risolto').length || 0;
  const urgentCount = tickets?.filter((t:any) => t.stato !== 'risolto' && (t.priorita === 'alta' || t.priorita === 'critica')).length || 0;
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
            ...
            {tickets && tickets.some((t:any) => !t.property_real_id && !t.property_mobile_id) && (
                <Button variant="destructive" className="bg-red-50 text-red-600 border border-red-200 hover:bg-red-100" onClick={deleteOrphans} disabled={isProcessing}>
                    <Trash2 className="w-4 h-4 mr-2"/> Pulizia
                </Button>
            )}
            ...
            <Dialog open={isImportOpen} onOpenChange={setIsImportOpen}>
                <DialogTrigger asChild>
                    <Button variant="outline" className="text-green-700 border-green-200 hover:bg-green-50 shadow-sm"><FileSpreadsheet className="w-4 h-4 mr-2"/> Importa</Button>
                </DialogTrigger>
                <DialogContent>
                    ...
                    <div className="space-y-4 py-4"><div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:bg-slate-50 transition-colors"><Input type="file" accept=".csv" onChange={(e) => setCsvFile(e.target.files?.[0] || null)} className="cursor-pointer"/><p className="text-xs text-gray-400 mt-2">Formato data: GG/MM/AAAA o AAAA-MM-GG</p></div></div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[600px]">
                <DialogHeader><DialogTitle>Nuovo Ticket</DialogTitle><DialogDescription>Segnala guasto o manutenzione.</DialogDescription></DialogHeader>
                <div className="space-y-4 mt-2">
                    ...
                    <div className="grid grid-cols-2 gap-4"><div className="grid gap-2"><Label>Titolo</Label><Input value={formData.titolo} onChange={e => setFormData({...formData, titolo: e.target.value})} placeholder="Es. Guasto..." /></div><div className="grid gap-2"><Label>Scadenza</Label><Input type="date" value={formData.scadenza} onChange={e => setFormData({...formData, scadenza: e.target.value})} /></div></div>
                    <div className="grid grid-cols-2 gap-4"><div className="grid gap-2"><Label>Priorità</Label><Select value={formData.priorita} onValueChange={v => setFormData({...formData, priorita: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="bassa">Bassa</SelectItem><SelectItem value="media">Media</SelectItem><SelectItem value="alta">Alta</SelectItem><SelectItem value="critica">Critica</SelectItem></SelectContent></Select></div>{targetType === 'real' && (<div className="grid gap-2"><Label className="flex items-center gap-2"><User className="w-4 h-4 text-green-600"/> Inquilino</Label><Select value={formData.booking_id} onValueChange={v => setFormData({...formData, booking_id: v})} disabled={!formData.target_id}><SelectTrigger><SelectValue placeholder="Seleziona..." /></SelectTrigger><SelectContent><SelectItem value="none">-- Nessuno --</SelectItem>{activeTenants?.map(t => <SelectItem key={t.id} value={t.id}>{t.nome_ospite}</SelectItem>)}</SelectContent></Select></div>)}</div>
      ...
      <Tabs defaultValue="open" value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value={activeTab} className="space-y-4">
            <div className="grid gap-4">
                {isLoading ? <p className="text-center py-8 text-gray-500">Caricamento ticket...</p> : paginatedTickets?.length === 0 ? (
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-2 mb-2 flex-wrap">
                                                ...
                                                {/* BADGE SCADENZA (UNICA AGGIUNTA) */}
                                                {ticket.scadenza && (
                                                    <Badge variant="secondary" className="flex items-center gap-1 bg-yellow-50 text-yellow-800 border-yellow-200">
                                                        <Clock className="w-3 h-3"/> Scade: {format(new Date(ticket.scadenza), 'dd MMM', {locale: it})}
                                                    </Badge>
                                                )}
                                            ...
                                            <p className="text-gray-700 text-sm mb-3 whitespace-pre-line">{ticket.descrizione}</p>
            ...
            {filteredTickets.length > 0 && (
                <div className="flex items-center justify-between p-3 border-t bg-slate-50 mt-4 rounded-lg">
                    <p className="text-xs text-gray-500 pl-2">Pagina {currentPage} di {totalPages}</p>
                    <div className="flex gap-2 pr-2">
                        <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}><ChevronLeft className="w-3 h-3 mr-1"/> Indietro</Button>
                        <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}>Avanti <ChevronRight className="w-3 h-3 ml-1"/></Button>
                    </div>
                </div>
            )}
        </TabsContent>
